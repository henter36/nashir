...
| 2 | `content_drafts.status` enum values | FAIL — `submitted`→`ready_for_review`; `withdrawn`→`rejected` (per OpenAPI: `rejected` covers both reviewer rejection and creator withdrawal); `archived` is a separate missing value; corrected 5-value set documented | **CLEAR** | Final — correction required | YES | SQL Migration Draft Authoring Correction Gate | **PASS** |
